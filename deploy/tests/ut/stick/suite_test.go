...
type StickSuite struct {
	suite.Suite
	test Test
}

func TestStick(t *testing.T) {
	suite.Run(t, new(StickSuite))
}
...
func (suite *StickSuite) BeforeTest(suiteName, testName string) {
...
func (suite *StickSuite) TearDownSuite() {
	_ = os.Unsetenv("POD_NAME")
}

func (suite *StickSuite) UseStickFixture() (eventChan chan k8ssync.SyncDataEvent) {
