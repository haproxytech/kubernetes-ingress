...
func (suite *StickSuite) TestStick() {
	eventChan := suite.UseStickFixture()
	...
	suite.Run("stick on src table stick-test if !localhost", func() {
		c := strings.Count(string(contents), "stick on src table stick-test if !localhost")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "stick match src table stick-updated")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "stick on src table stick-test if !localhost")
