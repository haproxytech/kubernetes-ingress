...
func (suite *FiltersSuite) TestFilters() {
	eventChan := suite.UseFiltersFixture()
	...
	suite.Run("filter compression", func() {
		c := strings.Count(string(contents), "filter compression")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "filter trace name diag")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "filter compression")
