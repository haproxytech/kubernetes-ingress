...
func (suite *HTTPChecksSuite) TestHTTPChecks() {
	eventChan := suite.UseHTTPChecksFixture()
	...
	suite.Run("http-check send meth GET uri /healthz", func() {
		c := strings.Count(string(contents), "http-check send meth GET uri /healthz")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "http-check send meth GET uri /ready")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "http-check send meth GET uri /healthz")
