...
type HTTPAfterResponsesSuite struct {
	suite.Suite
	test Test
}

func TestHTTPAfterResponses(t *testing.T) {
	suite.Run(t, new(HTTPAfterResponsesSuite))
}
...
func (suite *HTTPAfterResponsesSuite) BeforeTest(suiteName, testName string) {
...
func (suite *HTTPAfterResponsesSuite) TearDownSuite() {
	_ = os.Unsetenv("POD_NAME")
}

func (suite *HTTPAfterResponsesSuite) UseHTTPAfterResponsesFixture() (eventChan chan k8ssync.SyncDataEvent) {
		...
		Spec: v3.BackendSpec{
			Backend: models.Backend{
				...
				HTTPAfterResponseRuleList: models.HTTPAfterResponseRules{
					{
						Type:      "set-header",
						HdrName:   "X-After-Test",
						HdrFormat: "after-value",
					},
					{
						Type:         "set-status",
						Status:       503,
