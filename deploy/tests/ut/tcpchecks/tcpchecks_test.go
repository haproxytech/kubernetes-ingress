...
func (suite *TCPChecksSuite) TestTCPChecks() {
	eventChan := suite.UseTCPChecksFixture()
	...
	suite.Run("tcp-check connect port 8080", func() {
		c := strings.Count(string(contents), "tcp-check connect port 8080")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "tcp-check connect port 9090")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "tcp-check connect port 8080")
