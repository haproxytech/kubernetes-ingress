...
func (suite *TCPRequestsSuite) TestTCPRequests() {
	eventChan := suite.UseTCPRequestsFixture()
	...
	suite.Run("tcp-request content accept if { src 10.0.0.0/8 }", func() {
		c := strings.Count(string(contents), "tcp-request content accept if { src 10.0.0.0/8 }")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "tcp-request content reject if { src 192.168.0.0/16 }")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "tcp-request content accept if { src 10.0.0.0/8 }")
