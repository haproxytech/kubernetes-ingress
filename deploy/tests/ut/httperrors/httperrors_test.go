...
func (suite *HTTPErrorsSuite) TestHTTPErrors() {
	eventChan := suite.UseHTTPErrorsFixture()
	...
	suite.Run("http-error status 503", func() {
		c := strings.Count(string(contents), "http-error status 503")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "http-error status 429")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "http-error status 503")
