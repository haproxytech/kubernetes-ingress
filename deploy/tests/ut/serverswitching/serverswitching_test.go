...
func (suite *ServerSwitchingSuite) TestServerSwitching() {
	eventChan := suite.UseServerSwitchingFixture()
	...
	suite.Run("use-server SRV_1 if { path_beg /admin }", func() {
		c := strings.Count(string(contents), "use-server SRV_1 if { path_beg /admin }")
	...
	suite.Run("rule-list-only change still triggers config update", func() {
		...
		newCount := strings.Count(string(updatedContents), "use-server SRV_1 if { path_beg /api }")
		suite.Exactly(1, newCount, "expected new directive once after rule-list-only update, got %d", newCount)

		oldCount := strings.Count(string(updatedContents), "use-server SRV_1 if { path_beg /admin }")
