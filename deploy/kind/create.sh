...
sed 's/#GROUP#/zagreb/g' web/web-svc.yml | kubectl apply -f -

sed -e 's/#GROUP#/paris/g' -e  's/1 #NUMBER#/2/g' web/web-rc.yml | kubectl apply -f -
sed 's/#GROUP#/paris/g' web/web-svc.yml | kubectl apply -f -

sed -e 's/#GROUP#/waltham/g' -e  's/1 #NUMBER#/2/g' web/web-rc.yml | kubectl apply -f -
sed 's/#GROUP#/waltham/g' web/web-svc.yml | kubectl apply -f -

kubectl apply -f config/0.namespace.yaml
kubectl apply -f config/1.ingress.yaml
kubectl apply -f config/2.default.yaml
kubectl apply -f config/3.rbac.yaml
kubectl apply -f config/4.configmap.yaml
kubectl apply -f config/4.configmap.tcp.yaml
docker pull haproxytech/kubernetes-ingress:latest
kind --name=dev load docker-image haproxytech/kubernetes-ingress:latest
sed 's|TAG/IMAGE|haproxytech/kubernetes-ingress:latest|g' config/5.ingress-controller.yaml | kubectl apply -f -
