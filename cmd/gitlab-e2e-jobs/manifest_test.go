...
func TestLoadManifestReadsTheRepositoryManifest(t *testing.T) {
	...
	assert.Equal(t, []string{"parallel", "https", "sequential-1", "sequential-2"}, manifest.Jobs[0].Parts)
...
func TestLoadManifestRejectsAJobWithoutParts(t *testing.T) {
	path := writeManifest(t, "jobs:\n  - name: bad\n    stage: s\n    when: never\n")
	_, err := loadManifest(path)
	require.Error(t, err)
}
