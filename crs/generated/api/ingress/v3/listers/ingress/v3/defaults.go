...
type DefaultsLister interface {
	...
	List(selector labels.Selector) (ret []*v3.Defaults, err error)
...
// List lists all Defaults in the indexer.
func (s *defaultsLister) List(selector labels.Selector) (ret []*v3.Defaults, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v3.Defaults))
	})
	return ret, err
}
...
type DefaultsNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v3.Defaults, error)
...
// List lists all Defaults in the indexer for a given namespace.
func (s defaultsNamespaceLister) List(selector labels.Selector) (ret []*v3.Defaults, err error) {
...
// Get retrieves the Defaults from the indexer for a given namespace and name.
func (s defaultsNamespaceLister) Get(name string) (*v3.Defaults, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v3.Resource("defaults"), name)
	}
	return obj.(*v3.Defaults), nil
}
