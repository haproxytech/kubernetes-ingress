...
type GlobalLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Global, err error)
...
// List lists all Globals in the indexer.
func (s *globalLister) List(selector labels.Selector) (ret []*v1alpha2.Global, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Global))
	})
	return ret, err
}
...
type GlobalNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.Global, error)
...
// List lists all Globals in the indexer for a given namespace.
func (s globalNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Global, err error) {
...
// Get retrieves the Global from the indexer for a given namespace and name.
func (s globalNamespaceLister) Get(name string) (*v1alpha2.Global, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("global"), name)
	}
	return obj.(*v1alpha2.Global), nil
}
