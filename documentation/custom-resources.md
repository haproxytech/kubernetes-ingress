...
The frontend custom resource can be used to configure the four principal frontends `http`, `https`, `ssl` and `stats`. 
...
The frontends aforementioned are addressable by creating a Frontend custom resource and then assign it to the target frontend in the configmap with these four annotations:
...
The `ssl` frontend is the SSL passthrough frontend. It only exists while
`ssl-passthrough` is enabled, and it is fully generated by the controller, so its
custom resource behaves differently:

* scalar fields defined in the custom resource take precedence over the
  controller-generated ones (the http, https and stats frontends can only amend
  fields the controller does not set)
* the lists (acls, log targets, tcp request rules, ...) are merged according to
  an optional lists merge mode suffix on the annotation value:
  * `append` (default): the custom resource entries are appended after the
    controller-generated ones
  * `prepend`: the custom resource entries come first, before the
    controller-generated ones
  * `override`: a non-empty custom resource list replaces the corresponding
    controller-generated list, lists omitted by the custom resource are left as-is
* the frontend name, the defaults inheritance, the mode, the default backend and
  the binds are owned by the controller and are never amended
...
For the `ssl` frontend, the same frontend custom resource can be used, with an
optional lists merge mode suffix:
