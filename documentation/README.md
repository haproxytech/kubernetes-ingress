...
| [hard-stop-after](#hard-stop-after) :construction:(dev) | [time](#time) | "1h" |  |:large_blue_circle:|:white_circle:|:white_circle:|
...
| [enable-grpc](#enable-grpc) | [bool](#bool) | "false" |  |:large_blue_circle:|:large_blue_circle:|:large_blue_circle:|
...
| [servers-slots](#servers-slots) :construction:(dev) | number | 42 |  |:large_blue_circle:|:white_circle:|:white_circle:|
...
##### `enable-grpc`
  ...
  Enables GRPC to pods.

  Available on:  `configmap`  `ingress`  `service`

  :information_source: Enable HTTP/2 support for backend severs.

  :information_source: Enable GRPC proto
...
##### `servers-slots`
  ...
  Sets the number of servers slots that should be provisioned on a backend. If this number is greater than discovered endpoints, the remaining slots will be disabled (on stand by), ready to be used dynamically (without reload). if the number of server slots is lower than discovered endpoints then controller will create new server slots and reload.
