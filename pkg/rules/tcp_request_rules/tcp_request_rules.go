...
func Reconcile(client api.HAProxyClient, parentType rules.ParentType, parentName string, rules models.TCPRequestRules) error {
	...
	if len(diffRules) != 0 {
		...
		_ = client.TCPRequestRuleDeleteAll(string(parentType), parentName)
		// ... and create all the new ones if any
		for _, rule := range rules {
			errCreate := client.TCPRequestRuleCreate(string(parentType), parentName, *rule)
			if errCreate != nil {
