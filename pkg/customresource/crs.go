...
func (c GlobalCR) GetInformer(eventChan chan k8s.SyncDataEvent, factory informers.SharedInformerFactory) cache.SharedIndexInformer {
	informer := factory.Core().V1alpha1().Globals().Informer()

	sendToChannel := func(eventChan chan k8s.SyncDataEvent, object interface{}, status store.Status) {
		data := object.(*corev1alpha1.Global)
		logger.Debugf("%s %s: %s", data.GetNamespace(), status, data.GetName())
		if status == store.DELETED {
			eventChan <- k8s.SyncDataEvent{SyncType: k8s.CUSTOM_RESOURCE, CRKind: c.GetKind(), Namespace: data.GetNamespace(), Name: data.GetName(), Data: nil}
			return
		}
		eventChan <- k8s.SyncDataEvent{SyncType: k8s.CUSTOM_RESOURCE, CRKind: c.GetKind(), Namespace: data.GetNamespace(), Name: data.GetName(), Data: data}
	}
...
func (c GlobalCR) ProcessEvent(s *store.K8s, job k8s.SyncDataEvent) bool {
...
func (c DefaultsCR) GetInformer(eventChan chan k8s.SyncDataEvent, factory informers.SharedInformerFactory) cache.SharedIndexInformer {
...
func (c DefaultsCR) ProcessEvent(s *store.K8s, job k8s.SyncDataEvent) bool {
	ns := s.GetNamespace(job.Namespace)
	if job.Data == nil {
...
func (c BackendCR) GetInformer(eventChan chan k8s.SyncDataEvent, factory informers.SharedInformerFactory) cache.SharedIndexInformer {
...
func (c BackendCR) ProcessEvent(s *store.K8s, job k8s.SyncDataEvent) bool {
	ns := s.GetNamespace(job.Namespace)
	if job.Data == nil {
