...
func (s *Service) GetBackendName() (name string, err error) {
	...
	if svcPort.Name != "" {
		name = fmt.Sprintf("%s-%s-%s", s.resource.Namespace, s.resource.Name, svcPort.Name)
	} else {
		name = fmt.Sprintf("%s-%s-%s", s.resource.Namespace, s.resource.Name, strconv.Itoa(int(svcPort.Port)))
	}
	return
}
