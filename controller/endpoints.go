...
func (c *HAProxyController) alignHAproxySrvs(endpoints *Endpoints) (reload bool) {
...
func (c *HAProxyController) updateHAProxySrvs(oldEndpoints, newEndpoints *Endpoints) {
	if oldEndpoints.BackendName == "" {
		c.Logger.Errorf("No backend available for endpoints of service `%s`", oldEndpoints.Service.Value)
		return
	}
